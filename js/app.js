...
import {getSectionBlocks,renderCmsBlocks} from './cms-blocks.js';
...
const moduleExperience=ui.moduleExperience||{};
const rt=ui.runtime||{};
...
function seriesClosing(closing){if(!closing)return '';if(typeof closing==='string')return `<aside class="series-closing"><span class="eyebrow eyebrow--dark">${esc(ui.common?.carry||'')}</span><p>${esc(closing)}</p></aside>`;return `<aside class="series-closing"><span class="eyebrow eyebrow--dark">${esc(ui.components?.series?.closing||'')}</span><h3>${esc(closing.front||'')}</h3><p>${esc(closing.back||'')}</p></aside>`}
function cvComparison(items,key){if(!items?.length)return '';const i=Math.min(idx(key),items.length-1),x=items[i];const card=`<article class="flip-topic cv-flip"><div class="flip-topic__head"><span class="beta-card__number">${i+1} / ${items.length}</span><h3>${esc(x.topic)}</h3></div><button class="flip-card" type="button" data-flip-card aria-pressed="false" aria-label="${esc(rt.cvFlipAria||'')}: ${esc(x.topic)}"><span class="flip-card__inner"><span class="flip-card__face flip-card__front"><strong>${esc(rt.cvDont||'')}</strong><span>${esc(x.dont)}</span><small>${esc(rt.cvTapReplacement||'')} <b>↻</b></small></span><span class="flip-card__face flip-card__back"><strong>${esc(rt.cvDo||'')}</strong><span>${esc(x.do)}</span><small>${esc(ui.components?.flip?.tapFront||'')} <b>↺</b></small></span></span></button></article>`;return `<div class="paged-topic">${carousel(card,key,i,items.length)}${pager(key,i,items.length)}</div>`}
function cvTipsCards(items,key='cvTips'){if(!items?.length)return '';const i=Math.min(idx(key),items.length-1),x=items[i];const card=`<article class="flip-topic cv-tip-flip"><div class="flip-topic__head"><span class="beta-card__number">${i+1} / ${items.length}</span></div><button class="flip-card" type="button" data-flip-card aria-pressed="false" aria-label="${esc(rt.cvGenFlipAria||'')}"><span class="flip-card__inner"><span class="flip-card__face flip-card__front"><strong>${esc(rt.dontDo||'')}</strong><span>${esc(x.dont)}</span><small>${esc(rt.cvTapDoInstead||'')} <b>↻</b></small></span><span class="flip-card__face flip-card__back"><strong>${esc(rt.doIt||'')}</strong><span>${esc(x.do)}</span><small>${esc(ui.components?.flip?.tapFront||'')} <b>↺</b></small></span></span></button></article>`;return `<div class="paged-topic">${carousel(card,key,i,items.length)}${pager(key,i,items.length)}</div>`}
function cvTipsMarkup(){const tips=helyzeteim.cvTips||[],rule=helyzeteim.cvQuickRule;if(!tips.length&&!rule)return '';return `<section class="cv-tips"><span class="eyebrow eyebrow--dark">${esc(rt.cvTipsTitle||'')}</span>${tips.length?cvTipsCards(tips):''}${rule?`<aside class="series-closing"><strong>${esc(rt.quickRule||'')}</strong><p><b>${esc(rt.whatWrite||'')}</b> ${esc(rule.do)}</p><p><b>${esc(rt.whatNotWrite||'')}</b> ${esc(rule.dont)}</p></aside>`:''}</section>`}
...
function bodyScanExperience(){
 ...
 if(done)return `<article class="mindfulness-card mindfulness-complete"><div class="mindfulness-complete__icon">✓</div><h3>${esc(rt.bodyScanDoneTitle||'')}</h3><p>${esc(rt.bodyScanDoneText||'')}</p><button class="button" data-body-scan-start>${esc(rt.again||'')}</button></article>`;
 if(!active)return `<article class="mindfulness-card mindfulness-intro"><div class="mindfulness-icon">◌</div><p>${esc(rt.bodyScanIntro||'')}</p><button class="button" data-body-scan-start>${esc(rt.letsStart||'')}</button></article>`;
 const progress=((20-left)/20)*100;
 return `<article class="mindfulness-card body-scan-card" data-body-scan-card>${phaseDots(bodySteps.length,i)}<span class="eyebrow eyebrow--dark" data-body-scan-count>${i+1} / ${bodySteps.length}</span><h3 data-body-scan-zone>${esc(x.zone)}</h3><p data-body-scan-cue>${esc(x.cue)}</p><div class="body-scan-timer"><strong>${left}</strong><span>mp</span></div><button class="button button--ghost" data-body-scan-stop>${esc(ui.breathing?.stop||'')}</button></article>`;
}
function gratitudeExperience(item){
 const entries=state().mindfulness?.entries||['','',''];
 return `<article class="mindfulness-card"><div class="mindfulness-card__top"><span class="mindfulness-icon">♡</span><div><span class="eyebrow eyebrow--dark">${esc(item.tag)}</span><h3>${esc(item.title)}</h3></div></div><p class="mindfulness-description">${esc(item.description)}</p><div class="gratitude-fields">${entries.map((v,n)=>`<label><span>${esc(tpl(rt.thoughtLabel,{index:n+1}))}</span><input data-gratitude value="${esc(v)}" placeholder="${esc(rt.thoughtPlaceholder||'')}"></label>`).join('')}</div><div class="mindfulness-controls"><button class="button" data-gratitude-save>${esc(rt.saveThoughts||'')}</button></div></article>`;
}
...
function guidedAudioExperience(item,index){
 ...
 return `<article class="mindfulness-card guided-audio-card" data-guided-card><div class="guided-audio-intro"><div class="guided-audio-orb" data-audio-orb>${cfg.icon}</div><span class="eyebrow eyebrow--dark">${esc(tpl(rt.guidedAudioEyebrow,{duration:cfg.duration}))}</span><h3>${esc(item.title)}</h3><p>${esc(cfg.intro)}</p></div><audio id="${id}" preload="metadata" src="${cfg.src}"></audio><div class="guided-player"><div class="guided-progress" data-audio-seek data-audio-id="${id}" role="slider" tabindex="0" aria-label="${esc(rt.audioPositionAria||'')}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><i data-audio-progress></i></div><div class="guided-time"><span data-audio-current>0:00</span><span data-audio-duration>${cfg.duration}</span></div><div class="guided-controls"><button data-audio-restart data-audio-id="${id}" aria-label="${esc(rt.restartAria||'')}">↺</button><button class="guided-play" data-audio-toggle data-audio-id="${id}" aria-label="${esc(ui.components?.audio?.play||'')}">▶</button></div><label class="guided-volume"><span>${esc(rt.volume||'')}</span><input type="range" min="0" max="1" step="0.05" value="0.85" data-audio-volume data-audio-id="${id}"></label></div><aside class="mindfulness-takeaway"><strong>${esc(rt.carry||'')}</strong><span>${esc(cfg.done)}</span></aside>${guidedTranscriptMarkup(item,index)}</article>`;
}
function mindfulnessExperience(items){
 return `<div class="mindfulness-accordion">${items.map((x,i)=>{
  ...
  return `<details class="mindfulness-accordion__item" data-mindfulness-index="${i}" ${i===0?'open':''}><summary><span class="mindfulness-accordion__index">${String(i+1).padStart(2,'0')}</span><span class="mindfulness-accordion__copy"><small>${esc(x.tag||rt.exerciseFallback||'')}</small><strong>${esc(x.title)}</strong></span><span class="mindfulness-accordion__chevron" aria-hidden="true">⌄</span></summary><div class="mindfulness-accordion__body">${content}</div></details>`;
 }).join('')}</div>`;
}
const sectionCopy=(route,section)=>ui.sectionCopy?.[route]?.[section]||{};
function onmagamPage(){const s=sectionFor('onmagam');let c='';if(s==='preboarding'){const i=Math.min(idx('preboarding'),onmagam.preboardingData.length-1),x=onmagam.preboardingData[i];c=`${head(rt.preboardingTitle||'',rt.preboardingDescription||'')}${carousel(`<article class="qa-card"><h3>${esc(x.question)}</h3><div class="qa-answer"><p>${esc(x.answer)}</p></div></article>`,'preboarding',i,onmagam.preboardingData.length)}${pager('preboarding',i,onmagam.preboardingData.length)}`}
if(s==='survival'){const i=Math.min(idx('survival'),onmagam.survivalGuideData.length-1),x=onmagam.survivalGuideData[i];c=`${head(rt.survivalTitle||'',rt.survivalDescription||'')}${carousel(`<article class="month-card"><span class="month-badge">${x.month}. ${esc(rt.monthSuffix||'')}</span><h3>${esc(x.title)}</h3><p>${esc(x.content)}</p><aside><strong>${esc(rt.carry||'')}</strong><span>${esc(x.takeaway)}</span></aside></article>`,'survival',i,onmagam.survivalGuideData.length)}${pager('survival',i,onmagam.survivalGuideData.length)}`}
if(s==='marketfacts')c=`${head(rt.marketFactsTitle||'',rt.marketFactsDescription||'')}${pagedSimpleFlip(onmagam.marketFacts,'marketFacts',rt.marketFactsTitle||'',rt.marketFactsBack||'')}${seriesClosing(onmagam.marketFactsClosing)}`
if(s==='breathing'){const techniques=ui.breathing?.techniques||[];c=`${head(sectionCopy('onmagam','breathing').title||'',sectionCopy('onmagam','breathing').description||'')}<div class="breathing-accordion">${techniques.map((x,n)=>`<details class="breathing-accordion__item"><summary><span class="breathing-accordion__index">${String(n+1).padStart(2,'0')}</span><span class="breathing-accordion__copy"><small>${x.ratio}</small><strong>${x.name}</strong></span><span class="breathing-accordion__chevron" aria-hidden="true">⌄</span></summary><div class="breathing-accordion__body"><p>${x.description}</p><div class="breath-player"><div class="breath-player__title"><span class="eyebrow eyebrow--dark">${x.ratio}</span><strong data-breath-name>${x.name}</strong></div><div class="breath-orb is-ready" aria-live="polite"><div class="breath-orb__content"><strong data-breath-phase>${esc(ui.breathing?.ready||'')}</strong><span data-breath-count>–</span></div></div><div class="breath-player__actions"><button class="button" data-restart-breath data-in="${x.inhale}" data-hold="${x.hold}" data-out="${x.exhale}" data-name="${x.name}">${esc(ui.breathing?.start||'')}</button><button class="button button--secondary" data-stop-breath hidden>${esc(ui.breathing?.stop||'')}</button></div></div></div></details>`).join('')}</div>`}
if(s==='mindfulness')c=`${head(sectionCopy('onmagam','mindfulness').title||'',sectionCopy('onmagam','mindfulness').description||'')}${mindfulnessExperience(onmagam.mindfulnessExercises)}`
if(s==='careers')c=`${head(sectionCopy('onmagam','careers').title||'',sectionCopy('onmagam','careers').description||'')}${cards(onmagam.blueCollarExamples,x=>`<article class="career-card"><span class="career-from">${esc(x.from)}</span><div class="career-steps">${x.steps.map(y=>`<span>${esc(y)}</span>`).join('<b>›</b>')}</div><p>${esc(x.motivation)}</p><div class="chip-row">${(x.competencies||[]).map((y,n)=>`<button class="chip competency-chip" type="button" data-competency-profession="${esc(x.from)}" data-competency-index="${n}">${esc(y.name)}</button>`).join('')}</div></article>`)}${competencyDialog()}`;
if(s==='quiz')c=quiz(onmagam.onmagamQuizItems,'onmagamQuiz',sectionCopy('onmagam','quiz').title||rt.workplaceSituationsTitle||'',sectionCopy('onmagam','quiz').description||rt.workplaceSituationsDescription||'');return shell('onmagam',`${c}${renderCmsBlocks(getSectionBlocks(project,'onmagam',s))}`)}

function helyzeteimPage(){const s=sectionFor('helyzeteim');let c='';
 if(s==='world')c=`${head(sectionCopy('helyzeteim','world').title||'',sectionCopy('helyzeteim','world').description||'')}${pagedCard(helyzeteim.workWorldCards,'workWorldCards')}${seriesClosing(helyzeteim.workWorldClosing)}${quiz(helyzeteim.workWorldQuiz,'workWorldQuiz',sectionCopy('helyzeteim','world').quizTitle||'',sectionCopy('helyzeteim','world').quizDescription||'')}`;
 if(s==='jobhunt')c=`${head(sectionCopy('helyzeteim','jobhunt').title||'',sectionCopy('helyzeteim','jobhunt').description||'')}${pagedSimpleFlip(helyzeteim.jobHuntCards,'jobHuntCards',rt.tip||'',rt.meaning||'')}${seriesClosing(helyzeteim.jobHuntClosing)}`;
 if(s==='interview')c=`${head(sectionCopy('helyzeteim','interview').title||'',sectionCopy('helyzeteim','interview').description||'')}${pagedSimpleFlip(helyzeteim.interviewCards,'interviewCards',rt.interviewQuestion||'',rt.sampleAnswer||'')}${seriesClosing(helyzeteim.interviewClosing)}`;
 if(s==='cv')c=`${head(sectionCopy('helyzeteim','cv').title||'',sectionCopy('helyzeteim','cv').description||'')}${cvComparison(helyzeteim.cvCards,'cvCards')}${cvTipsMarkup()}`;
 if(s==='linkedin')c=`${head(sectionCopy('helyzeteim','linkedin').title||'',sectionCopy('helyzeteim','linkedin').description||'')}${pagedSimpleFlip(helyzeteim.linkedinCards,'linkedinCards',rt.tip||'',rt.whatToDo||'')}${seriesClosing(helyzeteim.linkedinClosing)}`;
 if(s==='firstday'){const checked=state().checklist||[];c=`${head(sectionCopy('helyzeteim','firstday').title||'',sectionCopy('helyzeteim','firstday').description||'')}<div class="checklist">${helyzeteim.firstDayChecklist.map(x=>`<label><input type="checkbox" data-check="${x.id}" ${checked.includes(x.id)?'checked':''}><span>${esc(x.text)}</span></label>`).join('')}</div>${quiz(helyzeteim.firstDayQuiz,'firstDayQuiz',sectionCopy('helyzeteim','firstday').quizTitle||'',sectionCopy('helyzeteim','firstday').quizDescription||'')}`}
 if(s==='redflags')c=`${head(sectionCopy('helyzeteim','redflags').title||'',sectionCopy('helyzeteim','redflags').description||'')}${accordion(helyzeteim.redFlags)}${quiz(helyzeteim.redFlagQuiz,'redFlagQuiz',sectionCopy('helyzeteim','redflags').quizTitle||'',sectionCopy('helyzeteim','redflags').quizDescription||'')}`;
 if(s==='legal')c=`${head(sectionCopy('helyzeteim','legal').title||'',sectionCopy('helyzeteim','legal').description||'')}${pagedFlip(helyzeteim.legalBasics,'legalBasicsCards',rt.essence||'',rt.goodToKnow||'')}${quiz(helyzeteim.legalBasicsQuiz,'legalQuiz',sectionCopy('helyzeteim','legal').quizTitle||'',sectionCopy('helyzeteim','legal').quizDescription||'')}`;
 ...
 return shell('helyzeteim',`${c}${renderCmsBlocks(getSectionBlocks(project,'helyzeteim',s))}`)}

function kapcsolataimPage(){const s=sectionFor('kapcsolataim');let c='';
 if(s==='twosides')c=`${head(sectionCopy('kapcsolataim','twosides').title||'',sectionCopy('kapcsolataim','twosides').description||'')}${pagedFlip(kapcsolataim.ketOldalData,'winwinTwoSides',rt.youngPerspective||'',rt.employerPerspective||'',{frontAudience:rt.genZExpectation||'',backAudience:rt.employerExpectation||''})}`;
 if(s==='generations')c=`${head(sectionCopy('kapcsolataim','generations').title||'',sectionCopy('kapcsolataim','generations').description||'')}${pagedSimpleFlip(kapcsolataim.generationCards,'generationCards',rt.topic||'',rt.message||'')}${seriesClosing(kapcsolataim.generationClosing)}`;
 if(s==='communication')c=`${head(sectionCopy('kapcsolataim','communication').title||'',sectionCopy('kapcsolataim','communication').description||'')}${pagedFlip(kapcsolataim.communicationFlips,'winwinCommunication',rt.amateur||'',rt.professional||'')}${quiz(kapcsolataim.komunikációQuiz,'commQuiz',sectionCopy('kapcsolataim','communication').quizTitle||'',sectionCopy('kapcsolataim','communication').quizDescription||'')}`;
 if(s==='phrases')c=`${head(sectionCopy('kapcsolataim','phrases').title||'',sectionCopy('kapcsolataim','phrases').description||'')}${pagedPhrase(kapcsolataim.proPhrases,'winwinPhrases')}${quiz(kapcsolataim.proSzóhasználatQuiz,'phraseQuiz',sectionCopy('kapcsolataim','phrases').quizTitle||'',sectionCopy('kapcsolataim','phrases').quizDescription||'')}`;
 if(s==='digital')c=`${head(sectionCopy('kapcsolataim','digital').title||'',sectionCopy('kapcsolataim','digital').description||'')}${pagedDigital(kapcsolataim.digitalBehavior,'winwinDigital')}${quiz(kapcsolataim.digitálisViselkedésQuiz,'digitalQuiz',sectionCopy('kapcsolataim','digital').quizTitle||'',sectionCopy('kapcsolataim','digital').quizDescription||'')}`;
 if(s==='glossary')c=`${head(sectionCopy('kapcsolataim','glossary').title||'',sectionCopy('kapcsolataim','glossary').description||'')}${pagedFlip(kapcsolataim.glossary,'winwinGlossary',rt.whatMeans||'',rt.whenYouMeet||'')}${quiz(kapcsolataim.fogalomtárQuiz,'glossaryQuiz',sectionCopy('kapcsolataim','glossary').quizTitle||'',sectionCopy('kapcsolataim','glossary').quizDescription||'')}`;
 return shell('kapcsolataim',`${c}${renderCmsBlocks(getSectionBlocks(project,'kapcsolataim',s))}`)}
function bonusPage(){const s=sectionFor('bonus');let c='';if(s==='materials'){const cats=[...new Set(bonus.bonusMaterials.map(x=>x.category))];c=`${head(ui.bonus?.materials?.title||'',ui.bonus?.materials?.description||'',ui.bonus?.materials?.eyebrow||'')}${cats.map(cat=>`<section class="bonus-section"><h3>${esc(cat)}</h3>${cards(bonus.bonusMaterials.filter(x=>x.category===cat),x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(ui.bonus?.available||''):(ui.bonus?.soon||'')}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}</section>`).join('')}`};if(s==='videos')c=`${head(ui.bonus?.videos?.title||'',ui.bonus?.videos?.description||'',ui.bonus?.videos?.eyebrow||'')}${cards(bonus.videoStories,x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(ui.bonus?.available||''):(ui.bonus?.soon||'')}</span><span class="eyebrow eyebrow--dark">${esc(x.profession)}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}`;return shell('bonus',`${c}${renderCmsBlocks(getSectionBlocks(project,'bonus',s))}`)}
...
function startBreath(){
 ...
 const phases=[
  {key:'in',label:ui.breathing?.inhale||'',seconds:breathConfig.inn},
  ...(breathConfig.hold>0?[{key:'hold',label:ui.breathing?.hold||'',seconds:breathConfig.hold}]:[]),
  {key:'out',label:ui.breathing?.exhale||'',seconds:breathConfig.out}
 ];
 let phaseIndex=0;
 const runPhase=()=>{
