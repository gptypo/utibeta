...
const sectionDetailPages=(route,section)=>{const pages=sectionData(route,section).detailPages;return Array.isArray(pages)?pages:[]};
const detailPageById=(route,section,id)=>sectionDetailPages(route,section).find(page=>page&&page.hidden!==true&&String(page.id||'')===String(id||''))||null;
...
function detailPageMarkup(route,section,page){if(!page)return'';const header=page.header||{},preset=PAGE_STYLE_PRESETS.has(String(page.stylePreset||'default'))?String(page.stylePreset||'default'):'default';const scope=`${section}:detail:${page.id}`;return `<section class="nested-detail section-appearance section-appearance--${esc(preset)} section-appearance--${modules[route]?.className||''}" data-detail-view="${esc(page.id)}"><button type="button" class="nested-detail__back" data-detail-back>${esc(page.backLabel||ui.common?.backToParent||'')}</button>${head(header.title||'',header.description||'',header.eyebrow||'')}${renderBlocks(route,scope,page.blocks||[])}</section>`}
...
function bonusPage(){const s=sectionFor('bonus'),page=sectionPage('bonus',s),header=page.header||{},labels=page.labels||{},detail=activeDetail('bonus',s);if(detail)return shell('bonus',detailPageMarkup('bonus',s,detail));let c='';if(s==='materials'){const cats=[...new Set(bonus.bonusMaterials.map(x=>x.category))];c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cats.map(cat=>`<section class="bonus-section"><h3>${esc(cat)}</h3>${cards(bonus.bonusMaterials.filter(x=>x.category===cat),x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}</section>`).join('')}`};if(s==='videos')c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cards(bonus.videoStories,x=>{const detailId=String(x.detailPage||'').trim(),linked=detailId&&detailPageById('bonus','videos',detailId);const inner=`<span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><span class="eyebrow eyebrow--dark">${esc(x.profession)}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p>${linked?`<span class="bonus-card__detail-link">${esc(ui.common?.open||'')} <b>→</b></span>`:''}`;return linked?`<button type="button" class="beta-card bonus-card bonus-card--link" data-detail-open="${esc(detailId)}" data-detail-section="videos">${inner}</button>`:`<article class="beta-card bonus-card">${inner}</article>`})}`;if(!c)c=genericSectionContent('bonus',s);return shell('bonus',`${c}${renderSectionBlocks('bonus',s)}`)}
