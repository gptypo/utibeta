...
function quizResultMarkup(items,key){const stats=quizStats(items,key),answers=quizAnswersFor(key);const wrong=Object.entries(answers).filter(([qi,oi])=>{const qIndex=Number(qi),item=items[qIndex];return item&&!item.options?.[Number(oi)]?.isOptimal});return `<section class="quiz-progress-panel" data-quiz-progress-key="${key}"><div class="quiz-progress-panel__head"><div><span class="eyebrow eyebrow--dark">Haladás</span><strong>${stats.percent}%</strong></div><div class="quiz-progress-track" aria-label="${stats.percent}% teljesítve"><i style="width:${stats.percent}%"></i></div></div><div class="quiz-progress-metrics"><span><b>${stats.correct}/${stats.total}</b><small>eredmény</small></span><span><b>${stats.answered}</b><small>megválaszolt</small></span><span><b>${stats.remainingToPerfect}</b><small>100%-hoz hátra</small></span></div><p class="quiz-development">${esc(quizEvaluation(stats))}</p>${wrong.length?`<details class="quiz-review"><summary>Hibás válaszok áttekintése (${wrong.length})</summary><div>${wrong.map(([qi,oi])=>{const q=items[Number(qi)],o=q.options[Number(oi)];return `<article><strong>${Number(qi)+1}. ${esc(q.situation||q.question||'Kérdés')}</strong><p><b>Válaszod:</b> ${esc(o.text)}</p><p>${esc(o.feedback||o.explanation||'')}</p></article>`}).join('')}</div></details>`:''}${stats.answered?`<button class="button button--secondary quiz-restart" type="button" data-quiz-restart="${key}">Kvíz újrakezdése</button>`:''}</section>`}
