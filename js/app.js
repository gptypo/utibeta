...
function quizResultMarkup(items,key){const stats=quizStats(items,key),answers=quizAnswersFor(key);const wrong=Object.entries(answers).filter(([qi,oi])=>{const qIndex=Number(qi),item=items[qIndex];return item&&!item.options?.[Number(oi)]?.isOptimal});return `<section class="quiz-progress-panel" data-quiz-progress-key="${key}"><div class="quiz-progress-panel__head"><div><span class="eyebrow eyebrow--dark">Haladás</span><strong>${stats.percent}%</strong></div><div class="quiz-progress-track" aria-label="${stats.percent}% teljesítve"><i style="width:${stats.percent}%"></i></div></div><div class="quiz-progress-metrics"><span><b>${stats.answered}</b><small>megválaszolt</small></span><span><b>${stats.correct}</b><small>helyes</small></span><span><b>${stats.skipped}</b><small>kihagyott</small></span></div><p class="quiz-development">${esc(quizEvaluation(stats))}</p>${wrong.length?`<details class="quiz-review"><summary>Hibás válaszok áttekintése (${wrong.length})</summary><div>${wrong.map(([qi,oi])=>{const q=items[Number(qi)],o=q.options[Number(oi)];return `<article><strong>${Number(qi)+1}. ${esc(q.situation||q.question||'Kérdés')}</strong><p><b>Válaszod:</b> ${esc(o.text)}</p><p>${esc(o.feedback||o.explanation||'')}</p></article>`}).join('')}</div></details>`:''}${stats.answered?`<button class="button button--secondary quiz-restart" type="button" data-quiz-restart="${key}">Kvíz újrakezdése</button>`:''}</section>`}
...
function moduleProgress(route){const entries=quizRegistry()[route]||[];if(!entries.length)return '';return `<section class="module-progress" aria-label="Kvízhaladás"><div class="module-progress__head"><span class="eyebrow eyebrow--dark">Összesített haladás</span><strong>Kvízek</strong></div><div class="module-progress__list">${entries.map(x=>{const st=quizStats(x.items,x.key);return `<div class="module-progress__item"><span>${esc(x.label)}</span><b>${st.answered}/${st.total}</b><i><em style="width:${st.percent}%"></em></i></div>`}).join('')}</div></section>`}
...
function homeProgressDashboard(){
 ...
 const rows=routes.map(route=>{
  ...
  totals.percent=totals.total?Math.round(totals.answered/totals.total*100):0;
  return {route,...totals};
 });
