...
function seriesClosing(closing){if(!closing)return '';if(typeof closing==='string')return `<aside class="series-closing"><span class="eyebrow eyebrow--dark">${esc(ui.common?.carry||'')}</span><p>${esc(closing)}</p></aside>`;return `<aside class="series-closing"><span class="eyebrow eyebrow--dark">Záró gondolat</span><h3>${esc(closing.front||'')}</h3><p>${esc(closing.back||'')}</p></aside>`}
...
function helyzeteimPage(){const s=sectionFor('helyzeteim');let c='';
 if(s==='world')c=`${head(sectionCopy('helyzeteim','world').title||'',sectionCopy('helyzeteim','world').description||'')}${pagedCard(helyzeteim.workWorldCards,'workWorldCards')}${seriesClosing(helyzeteim.workWorldClosing)}${quiz(helyzeteim.workWorldQuiz,'workWorldQuiz','Ellenőrző kvíz','Valósághű helyzetek az első hetekből.')}`;
 if(s==='jobhunt')c=`${head(sectionCopy('helyzeteim','jobhunt').title||'',sectionCopy('helyzeteim','jobhunt').description||'')}${pagedSimpleFlip(helyzeteim.jobHuntCards,'jobHuntCards','Tipp','Mit jelent?')}${seriesClosing(helyzeteim.jobHuntClosing)}`;
 if(s==='interview')c=`${head(sectionCopy('helyzeteim','interview').title||'',sectionCopy('helyzeteim','interview').description||'')}${pagedSimpleFlip(helyzeteim.interviewCards,'interviewCards','Interjúkérdés','Minta válasz')}${seriesClosing(helyzeteim.interviewClosing)}`;
 ...
 if(s==='linkedin')c=`${head(sectionCopy('helyzeteim','linkedin').title||'',sectionCopy('helyzeteim','linkedin').description||'')}${pagedSimpleFlip(helyzeteim.linkedinCards,'linkedinCards','Tipp','Mit tegyél?')}${seriesClosing(helyzeteim.linkedinClosing)}`;
 ...
 if(s==='redflags')c=`${head(sectionCopy('helyzeteim','redflags').title||'',sectionCopy('helyzeteim','redflags').description||'')}${accordion(helyzeteim.redFlags)}${quiz(helyzeteim.redFlagQuiz,'redFlagQuiz','Ellenőrző kvíz','Fedezd fel a biztonságos, professzionális reakciókat.')}`;
 if(s==='legal')c=`${head(sectionCopy('helyzeteim','legal').title||'',sectionCopy('helyzeteim','legal').description||'')}${pagedFlip(helyzeteim.legalBasics,'legalBasicsCards','Lényeg','Jó tudni')}${quiz(helyzeteim.legalBasicsQuiz,'legalQuiz','Ellenőrző kvíz','Mit érdemes tenni aláírás és döntés előtt?')}`;
...
function kapcsolataimPage(){const s=sectionFor('kapcsolataim');let c='';
 if(s==='twosides')c=`${head(sectionCopy('kapcsolataim','twosides').title||'',sectionCopy('kapcsolataim','twosides').description||'')}${pagedFlip(kapcsolataim.ketOldalData,'winwinTwoSides','Fiatal nézőpont','Munkaadói nézőpont',{frontAudience:'Z-generáció elvárása',backAudience:'Munkaadó elvárása'})}`;
 if(s==='generations')c=`${head(sectionCopy('kapcsolataim','generations').title||'',sectionCopy('kapcsolataim','generations').description||'')}${pagedSimpleFlip(kapcsolataim.generationCards,'generationCards','Téma','Üzenet')}${seriesClosing(kapcsolataim.generationClosing)}`;
 if(s==='communication')c=`${head(sectionCopy('kapcsolataim','communication').title||'',sectionCopy('kapcsolataim','communication').description||'')}${pagedFlip(kapcsolataim.communicationFlips,'winwinCommunication','Amatőr','Profi')}${quiz(kapcsolataim.komunikációQuiz,'commQuiz','Ellenőrző kvíz','Keresd a mindkét fél számára működő megoldást.')}`;
 if(s==='phrases')c=`${head(sectionCopy('kapcsolataim','phrases').title||'',sectionCopy('kapcsolataim','phrases').description||'')}${pagedPhrase(kapcsolataim.proPhrases,'winwinPhrases')}${quiz(kapcsolataim.proSzóhasználatQuiz,'phraseQuiz','Ellenőrző kvíz','Válaszd a konkrét és együttműködő megfogalmazást.')}`;
 if(s==='digital')c=`${head(sectionCopy('kapcsolataim','digital').title||'',sectionCopy('kapcsolataim','digital').description||'')}${pagedDigital(kapcsolataim.digitalBehavior,'winwinDigital')}${quiz(kapcsolataim.digitálisViselkedésQuiz,'digitalQuiz','Ellenőrző kvíz','Találd meg a megfelelő csatornát és hangnemet.')}`;
 if(s==='glossary')c=`${head(sectionCopy('kapcsolataim','glossary').title||'',sectionCopy('kapcsolataim','glossary').description||'')}${pagedFlip(kapcsolataim.glossary,'winwinGlossary','Mit jelent?','Mikor találkozol vele?')}${quiz(kapcsolataim.fogalomtárQuiz,'glossaryQuiz','Ellenőrző kvíz','Használd magabiztosan az alapfogalmakat.')}`;
