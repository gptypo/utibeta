...
import {markVisited,markCompleted,clearCompleted,progressSummary} from './progress-engine.js';
...
const searchCategories=[...new Set(contentSearchIndex.map(x=>x.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'hu'));
const searchTags=[...new Set(contentSearchIndex.flatMap(x=>x.tags||[]).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'hu'));
...
const sectionDetailPages=(route,section)=>{
 ...
 const scan=value=>{
  ...
  if(itemId&&detail&&typeof detail==='object'&&!Array.isArray(detail)&&detail.enabled===true&&isPublished(detail)){pages.push({...detail,id:itemId,hidden:false,embedded:true})}
  for(const [key,child] of Object.entries(value))if(key!=='detail'&&key!=='detailPages')scan(child);
 };
 scan(data);
 if(Array.isArray(data.detailPages))for(const page of data.detailPages)if(page&&isPublished(page))pages.push(page);
...
const dynamicBlocks=(route,section)=>activeSectionBlocks(route,section).filter(block=>block&&typeof block==='object'&&isPublished(block));
...
function dynamicQuizEntries(route){const out=[];for(const [sectionId,title] of sectionMap[route]||[]){getSectionBlocks(project,route,sectionId).forEach((block,index)=>{if(!isPublished(block)||String(block?.type||'').toLowerCase()!=='quiz'||!Array.isArray(block.items)||!block.items.length)return;out.push({key:interactiveBlockKey(route,sectionId,index,block,'cmsQuiz'),label:block.title||title,items:block.items});});for(const page of sectionDetailPages(route,sectionId)){if(page?.hidden===true||!Array.isArray(page.blocks))continue;const scope=`${sectionId}:detail:${page.id}`;page.blocks.forEach((block,index)=>{if(!isPublished(block)||String(block?.type||'').toLowerCase()!=='quiz'||!Array.isArray(block.items)||!block.items.length)return;out.push({key:interactiveBlockKey(route,scope,index,block,'cmsQuiz'),label:block.title||page.header?.title||title,items:block.items});});}}return out}
...
const sectionMap=Object.fromEntries(project.navigation.map(module=>[module.id,module.sections.filter(section=>isPublished(sectionPage(module.id,section.id))).map(section=>[section.id,section.title])]));
...
function searchMarkup(query='',filters={}){const results=searchContent(contentSearchIndex,query,12,filters);if(String(query||'').trim().length<2&&!filters.tag&&!filters.category)return `<p class="content-search__hint">${esc(ui.search?.hint||'')}</p>`;if(!results.length)return `<p class="content-search__empty">${esc(ui.search?.empty||'')}</p>`;return results.map(item=>`<button type="button" class="content-search__result" data-search-route="${esc(item.route)}" data-search-section="${esc(item.section)}" data-search-detail="${esc(item.detail||'')}"><span>${esc(item.subtitle||'')}${item.category?` · ${esc(item.category)}`:''}</span><strong>${esc(item.title)}</strong><b aria-hidden="true">→</b></button>`).join('')}
...
function recordQuizAnswer(key,questionIndex,optionIndex){const st=state(),current={...(st.quizAnswers?.[key]||{})};current[questionIndex]=optionIndex;save({quizAnswers:{...(st.quizAnswers||{}),[key]:current}});const items=quizItemsByKey(key)||[];if(items.length&&Object.keys(current).filter(q=>Number(q)<items.length).length>=items.length)markCompleted(`quiz:${key}`,{type:'quiz',route:state().route,section:sectionFor(state().route)})}
...
function resetQuiz(key){const st=state(),qa={...(st.quizAnswers||{})},vw={...(st.viewed||{})},indices={...(st.indices||{})};delete qa[key];Object.keys(vw).filter(k=>k.startsWith(`${key}-`)).forEach(k=>delete vw[k]);indices[key]=0;save({quizAnswers:qa,viewed:vw,indices});clearCompleted(`quiz:${key}`);render(state().route,{preserveScroll:true})}
...
function homeProgressDashboard(){
 const routes=['onmagam','helyzeteim','kapcsolataim','bonus'];
...
document.addEventListener('change',e=>{const input=e.target.closest?.('[data-dynamic-checklist]');if(!input)return;const key=input.dataset.dynamicChecklist,id=input.dataset.itemId,st=state(),all={...(st.dynamicChecklist||{})},set=new Set(all[key]||[]);input.checked?set.add(id):set.delete(id);all[key]=[...set];save({dynamicChecklist:all});const group=[...document.querySelectorAll(`[data-dynamic-checklist="${CSS.escape(key)}"]`)];if(group.length&&group.every(x=>x.checked))markCompleted(`checklist:${key}`,{type:'checklist',route:state().route,section:sectionFor(state().route)});else clearCompleted(`checklist:${key}`)});
