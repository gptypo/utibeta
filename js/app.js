...
function bonusPage(){const s=sectionFor('bonus'),page=sectionPage('bonus',s),header=page.header||{},labels=page.labels||{},detail=activeDetail('bonus',s);if(detail)return shell('bonus',detailPageMarkup('bonus',s,detail));let c='';if(s==='materials'){const cats=[...new Set(bonus.bonusMaterials.map(x=>x.category))];c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cats.map(cat=>`<section class="bonus-section"><h3>${esc(cat)}</h3>${cards(bonus.bonusMaterials.filter(x=>x.category===cat),x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}</section>`).join('')}`};if(s==='videos')c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cards(bonus.videoStories,x=>{const detailId=x?.detail?.enabled===true?String(x.id||'').trim():String(x.detailPage||'').trim(),linked=detailId&&detailPageById('bonus','videos',detailId);const inner=`<span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><span class="eyebrow eyebrow--dark">${esc(x.profession)}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p>${linked?`<span class="bonus-card__detail-link">${esc(ui.common?.open||'')} <b>→</b></span>`:''}`;return linked?`<button type="button" class="beta-card bonus-card bonus-card--link" data-detail-open="${esc(detailId)}" data-detail-section="videos">${inner}</button>`:`<article class="beta-card bonus-card">${inner}</article>`})}`;if(!c)c=genericSectionContent('bonus',s);return shell('bonus',`${c}${renderSectionBlocks('bonus',s)}`)}
