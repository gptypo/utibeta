...
import {getSectionBlocks,renderCmsBlock} from './cms-blocks.js';
...
const dynamicBlocks=(route,section)=>getSectionBlocks(project,route,section).filter(block=>block&&typeof block==='object'&&!block.hidden);
function dynamicQuizEntries(route){const out=[];for(const [sectionId,title] of sectionMap[route]||[]){dynamicBlocks(route,sectionId).forEach((block,index)=>{if(String(block.type||'').toLowerCase()!=='quiz'||!Array.isArray(block.items)||!block.items.length)return;out.push({key:interactiveBlockKey(route,sectionId,index,block,'cmsQuiz'),label:block.title||title,items:block.items});});}return out}
...
function renderDynamicInteractiveBlock(route,section,block,index){
 ...
 if(type==='quiz'){
  ...
  return `<section class="cms-interactive cms-interactive--quiz">${quiz(items,key,block.title||'',block.description||'',block.eyebrow||rt.quizLabel||'')}</section>`;
 }
 if(type==='carousel'){
  ...
  return `<section class="cms-interactive cms-interactive--carousel">${block.title||block.description||block.eyebrow?head(block.title||'',block.description||'',block.eyebrow||''):''}${carousel(card,key,i,items.length)}<div class="pager"><button type="button" data-dynamic-step="${esc(key)}" data-dir="-1" ${i===0?'disabled':''}>← ${esc(rt.previousPage||'')}</button><span>${i+1} / ${items.length}</span><button type="button" data-dynamic-step="${esc(key)}" data-dir="1" ${i===items.length-1?'disabled':''}>${esc(rt.nextPage||'')} →</button></div></section>`;
 }
 if(type==='flipcards'){
  ...
  return `<section class="cms-interactive cms-interactive--flipcards">${block.title||block.description||block.eyebrow?head(block.title||'',block.description||'',block.eyebrow||''):''}${carousel(card,key,i,items.length)}<div class="pager"><button type="button" data-dynamic-step="${esc(key)}" data-dir="-1" ${i===0?'disabled':''}>← ${esc(rt.previousPage||'')}</button><span>${i+1} / ${items.length}</span><button type="button" data-dynamic-step="${esc(key)}" data-dir="1" ${i===items.length-1?'disabled':''}>${esc(rt.nextPage||'')} →</button></div></section>`;
 }
 if(type==='checklist'){
  ...
  return `<section class="cms-interactive cms-interactive--checklist">${block.title||block.description||block.eyebrow?head(block.title||'',block.description||'',block.eyebrow||''):''}<div class="checklist">${items.map((item,n)=>{const id=String(item.id||n);return `<label><input type="checkbox" data-dynamic-checklist="${esc(key)}" data-item-id="${esc(id)}" ${checked.includes(id)?'checked':''}><span>${esc(item.text)}</span></label>`}).join('')}</div></section>`;
 }
 if(type==='steps'){
  const items=Array.isArray(block.items)?block.items.filter(item=>item&&(item.title||item.body)):[];if(!items.length)return'';
  return `<section class="cms-interactive cms-interactive--steps">${block.title||block.description||block.eyebrow?head(block.title||'',block.description||'',block.eyebrow||''):''}<div class="cms-steps">${items.map((item,n)=>`<article><span>${n+1}</span><div><h3>${esc(item.title||'')}</h3>${item.body?`<p>${esc(item.body)}</p>`:''}</div></article>`).join('')}</div></section>`;
 }
 return null;
}
function renderSectionBlocks(route,section){const blocks=getSectionBlocks(project,route,section);if(!Array.isArray(blocks)||!blocks.length)return'';const markup=blocks.map((block,index)=>{const interactive=renderDynamicInteractiveBlock(route,section,block,index);return interactive===null?renderCmsBlock(block):interactive}).filter(Boolean).join('');return markup?`<div class="cms-blocks">${markup}</div>`:''}
function dynamicPagerEntries(route,section){const entries=[];dynamicBlocks(route,section).forEach((block,index)=>{const type=String(block.type||'').toLowerCase();if(type==='quiz'&&Array.isArray(block.items)&&block.items.length)entries.push([interactiveBlockKey(route,section,index,block,'cmsQuiz'),()=>block.items.length]);if(['carousel','flipcards'].includes(type)&&Array.isArray(block.items)&&block.items.length)entries.push([interactiveBlockKey(route,section,index,block,type==='carousel'?'cmsCarousel':'cmsFlip'),()=>block.items.length]);});return entries}
...
const defaultState=()=>({schema:STATE_SCHEMA,route:'home',lastRoute:'onmagam',sections:{},indices:{},viewed:{},quizAnswers:{},checklist:[],activeBreath:null,mindfulness:{},dynamicChecklist:{}});
...
function bonusPage(){const s=sectionFor('bonus'),page=sectionPage('bonus',s),header=page.header||{},labels=page.labels||{};let c='';if(s==='materials'){const cats=[...new Set(bonus.bonusMaterials.map(x=>x.category))];c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cats.map(cat=>`<section class="bonus-section"><h3>${esc(cat)}</h3>${cards(bonus.bonusMaterials.filter(x=>x.category===cat),x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}</section>`).join('')}`};if(s==='videos')c=`${head(header.title||'',header.description||'',header.eyebrow||'')}${cards(bonus.videoStories,x=>`<article class="beta-card bonus-card"><span class="status-pill ${x.status==='live'?'is-live':''}">${x.status==='live'?(labels.available||''):(labels.soon||'')}</span><span class="eyebrow eyebrow--dark">${esc(x.profession)}</span><h3>${esc(x.title)}</h3><p>${esc(x.description)}</p></article>`)}`;if(!c)c=genericSectionContent('bonus',s);return shell('bonus',`${c}${renderSectionBlocks('bonus',s)}`)}
